<?php
    ...
    if(!empty($full_url) && filter_var($full_url, FILTER_VALIDATE_URL)){
        ...
        if(mysqli_num_rows($sql) > 0){
			...
			/*echo "INSERT INTO url (user_id,full_url, shorten_url, clicks) 
                                         VALUES ('{$_SESSION['id']}','{$full_url}', '{$ran_url}', '0')";*/
            $sql2 = mysqli_query($conn, "INSERT INTO url (users_id,full_url, shorten_url, clicks) 
                                         VALUES ('{$_SESSION['id']}','{$full_url}', '{$ran_url}', '0')");
            if($sql2){
